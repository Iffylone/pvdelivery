...
// Tracking de caja activa — cliente/rider solo pueden entrar si hay caja online
let cajaConectadaWs = null; // referencia al WS de la caja activa

app.get('/api/caja/estado', (q, r) => {
  const activa = cajaConectadaWs !== null && cajaConectadaWs.readyState === WebSocket.OPEN;
  r.json({ activa, ts: Date.now() });
});

wss.on('connection', async ws => {
  ws.isAlive = true;
  ws.rol = null; // se setea cuando el cliente manda IDENTIFICAR_ROL
  ...
  ws.on('close', () => {
    C.delete(ws);
    // Si era la caja la que se desconectó, notificar a todos
    if (ws.rol === 'caja') {
      cajaConectadaWs = null;
      bcastLight({ tipo: 'CAJA_ESTADO', activa: false });
  ...
  ws.on('message', data => {
    try {
      ...
      // Identificación de rol al conectar
      ...
      // Reenviar ubicación del rider a todos
      if (msg.tipo === 'UBICACION_REP') {
        const t = JSON.stringify(msg);
        C.forEach(w => { if (w !== ws && w.readyState === WebSocket.OPEN) try { w.send(t); } catch(e){} });
      }
    } catch(e) {}
...
app.post('/api/mp/preference', async (q, r) => {
  ...
  try {
    ...
    const baseUrl = q.headers.origin || `https://${q.headers.host}` || 'https://pvdelivery-production.up.railway.app';
    const preferenceData = {
      items: (items || []).map(it => ({
        id:         String(it.id || pedidoId || Date.now()),
        title:      String(it.title || 'Pedido PVDelivery').substring(0, 256),
        quantity:   Math.max(1, parseInt(it.quantity) || 1),
        unit_price: Math.round(parseFloat(it.unit_price) || 0),
        currency_id: 'ARS',
        description: String(it.description || '').substring(0, 256)
      ...
      back_urls: {
        success: back_urls?.success || `${baseUrl}/?mp=success`,
        failure: back_urls?.failure || `${baseUrl}/?mp=failure`,
        pending: back_urls?.pending || `${baseUrl}/?mp=pending`
      },
      auto_return: 'approved',
      external_reference: external_reference || String(pedidoId || Date.now()),
      notification_url: `https://pvdelivery-production.up.railway.app/api/mp/webhook`,
...
// ── Health check liviano para UptimeRobot ─────────────────────
// Monitorear en: https://pvdelivery.onrender.com/health
app.get('/health', (q, r) => {
  r.json({
    ok: true,

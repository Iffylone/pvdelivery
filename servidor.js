...
app.post('/api/mp/preference', async (q, r) => {
  ...
  try {
    ...
    // Construir items válidos — al menos uno con precio > 0
    let mpItems = (items || []).map(it => ({
      id:         String(it.id || pedidoId || Date.now()),
      title:      String(it.title || 'Ítem').substring(0, 256),
      quantity:   Math.max(1, parseInt(it.quantity) || 1),
      unit_price: Math.max(1, Math.round(parseFloat(it.unit_price) || 0)),
      currency_id: 'ARS',
      description: String(it.description || '').substring(0, 256)
    })).filter(it => it.unit_price > 0);
    
    // Fallback: si no hay items válidos, usar el total del pedido como un ítem genérico
    if (!mpItems.length) {
      const totalPedido = (items || []).reduce((s, it) => s + (Math.round(parseFloat(it.unit_price || 0)) * Math.max(1, parseInt(it.quantity) || 1)), 0);
      mpItems = [{ id: String(pedidoId || Date.now()), title: `Pedido #${String(pedidoId).padStart(4,'0')}`, quantity: 1, unit_price: Math.max(1, totalPedido), currency_id: 'ARS' }];
    }

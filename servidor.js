...
async function initDB() {
  ...
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS repartidores(id BIGINT PRIMARY KEY, nombre TEXT, tel TEXT, com NUMERIC);
...
async function getDB() {
  ...
  return {
    ...
    repartidores: re.rows.map(r => ({ id:Number(r.id), nombre:r.nombre, tel:r.tel, com:Number(r.com) })),
...
app.post('/api/repartidores', async (q, r) => {
  try {
    const { nombre, tel, com } = q.body;
    const id = Date.now();
    if (useDB) await pool.query('INSERT INTO repartidores VALUES($1,$2,$3,$4) ON CONFLICT(id) DO UPDATE SET nombre=$2,tel=$3,com=$4', [id, nombre, tel||'', com||0]);
    else ram.repartidores.push({ id, nombre, tel:tel||'', com:com||0 });
    await bcast('DATOS_ACTUALIZADOS', {});
    r.json({ ok: true, rep: { id, nombre, tel, com } });
  } catch(e) { console.error(e); r.status(500).json({ error: e.message }); }
});

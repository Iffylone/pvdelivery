...
async function initDB() {
  ...
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS repartidores(id INTEGER PRIMARY KEY, nombre TEXT, dni TEXT, tel TEXT, com NUMERIC, foto TEXT, activo BOOLEAN DEFAULT true);
    ...
    CREATE TABLE IF NOT EXISTS clientes    (id BIGINT PRIMARY KEY, nombre TEXT, tel TEXT, dir TEXT, created_at TIMESTAMPTZ DEFAULT NOW());
    CREATE TABLE IF NOT EXISTS reportes    (fecha TEXT PRIMARY KEY, data JSONB);
  `);
...
app.post('/api/repartidores', async (q, r) => {
  try {
    const { nombre, dni, tel, com, foto, vehiculo } = q.body;
    let newId;
    if (useDB) {
...
app.put('/api/repartidores/:id', async (q, r) => {
  try {
    const id = parseInt(q.params.id);
    const { nombre, dni, tel, com, foto, vehiculo, activo } = q.body;
    if (useDB) {
...
app.post('/api/clientes', async (q, r) => {
  try {
    const { nombre, tel, dir } = q.body;
    if (!tel) return r.status(400).json({ error: 'tel requerido' });
    const id = Date.now();
    if (useDB) {
      await pool.query('INSERT INTO clientes(id,nombre,tel,dir) VALUES($1,$2,$3,$4) ON CONFLICT DO NOTHING', [id,nombre||'',tel,dir||'']);
    }
    r.json({ ok:true, id });
  } catch(e) { r.status(500).json({ error: e.message }); }
});

app.get('/api/clientes/buscar', async (q, r) => {
  try {
    const tel = (q.query.tel||'').replace(/\D/g,'');
    if (!tel) return r.json({ found:false });
    if (useDB) {
      const res = await pool.query('SELECT * FROM clientes WHERE REGEXP_REPLACE(tel,\'[^0-9]\',\'\',\'g\')=$1 LIMIT 1',[tel]);
      if (res.rows.length) return r.json({ found:true, cliente: res.rows[0] });
    }
    r.json({ found:false });
  } catch(e) { r.status(500).json({ error: e.message }); }
});

app.post('/api/reportes', async (q, r) => {
  try {
    const { fecha, data } = q.body;
    if (useDB) {
      await pool.query('INSERT INTO reportes(fecha,data) VALUES($1,$2) ON CONFLICT(fecha) DO UPDATE SET data=$2', [fecha, JSON.stringify(data)]);
    }
    r.json({ ok:true });
  } catch(e) { r.status(500).json({ error: e.message }); }
});

app.get('/api/reportes', async (q, r) => {
  try {
    if (useDB) {
      const res = await pool.query('SELECT fecha,data FROM reportes ORDER BY fecha DESC LIMIT 90');
      return r.json({ reportes: res.rows.map(row=>({...row.data, fecha:row.fecha})) });
    }
    r.json({ reportes:[] });
  } catch(e) { r.status(500).json({ error: e.message }); }
});

...
async function initDB() {
  ...
  await pool.query(`INSERT INTO config(key,value) VALUES('negocio','Mi Negocio'),('contador','1') ON CONFLICT(key) DO NOTHING`);
...
app.put('/api/pedidos/:id/estado', async (q, r) => {
  try {
    ...
    // Permitir actualizar repartidor desde cocina
...
app.post('/api/repartidores', async (q, r) => {
  try {
    ...
    // Generar ID autonumérico de 4 dígitos
    let newId;
    if (useDB) {
      ...
      if (newId > 9999) newId = parseInt(mx) + 1; // overflow graceful
      await pool.query(
        'INSERT INTO repartidores(id,nombre,dni,tel,com,foto,activo) VALUES($1,$2,$3,$4,$5,$6,true) ON CONFLICT(id) DO UPDATE SET nombre=$2,dni=$3,tel=$4,com=$5,foto=$6',
        [newId, nombre, dni||'', tel||'', com||0, foto||null]
      );
    } else {
      const mx = ram.repartidores.reduce((m,r) => Math.max(m, r.id||0), 999);
      newId = Math.max(mx + 1, 1000);
      ram.repartidores.push({ id:newId, nombre, dni:dni||'', tel:tel||'', com:com||0, foto:foto||null, activo:true, lat:null, lng:null });
    }
...
app.put('/api/repartidores/:id/ubicacion', async (q, r) => {
  try {
    ...
    // Broadcast liviano solo con la ubicación — no recarga toda la DB

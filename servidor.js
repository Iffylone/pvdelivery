...
app.post('/api/mp/preference', async (q, r) => {
  ...
  try {
    ...
    const buildItems = (rawItems) => rawItems.map(it => {
      ...
      const price = Number.isFinite(rawPrice) && rawPrice > 0 ? Math.round(rawPrice) : 1;
...
// PAYMENT_REQUIRED_FLOW
// El pedido NO debe entrar al local hasta payment.status === 'approved'
// Mercado Pago webhook libera el pedido al panel del negocio.
